...
get_memory_usage() {
    ...
    local container_memory_valid=false
    ...
    if [[ -f /sys/fs/cgroup/memory.max ]]; then
        # CGroup v2
        cgroup_version="v2"
        local max_raw=$(cat /sys/fs/cgroup/memory.max 2>/dev/null || echo "max")
        if [[ "$max_raw" != "max" && "$max_raw" =~ ^[0-9]+$ ]]; then
            ...
            if [[ "$cgroup_limit" -gt 0 ]]; then
                container_memory_valid=true
            fi
        fi
    elif [[ -f /sys/fs/cgroup/memory/memory.limit_in_bytes ]]; then
        # CGroup v1
        cgroup_version="v1"
        local limit_raw=$(cat /sys/fs/cgroup/memory/memory.limit_in_bytes 2>/dev/null || echo "9223372036854771712")
        # 排除巨大的未限制值（典型值：9223372036854771712）
    ...
    # ==================== 2. 容器环境优先使用cgroup数据 ====================
    if [[ "$is_container" == "true" && "$container_memory_valid" == "true" ]]; then
        log "使用CGroup ${cgroup_version} 内存数据 (容器环境)"
        ...
        # 验证数据合理性
        if [[ $total -gt 0 && $used -ge 0 && $free -ge 0 ]]; then
            # 计算使用百分比
            usage_percent=$(echo "scale=1; $used * 100 / $total" | bc 2>/dev/null || echo "0")
            
            log "容器内存: 总=${total}KB, 已用=${used}KB, 可用=${free}KB, 使用率=${usage_percent}%"
            
            echo "{\"total\":$total,\"used\":$used,\"free\":$free,\"usage_percent\":$usage_percent,\"source\":\"cgroup_${cgroup_version}\",\"is_container\":true}"
            return 0
        fi
    fi
    ...
    if [[ $total -eq 0 ]] && [[ -f /proc/meminfo ]]; then
        ...
        # 计算实际可用内存（包括可回收的内存）
        if [[ "$total" =~ ^[0-9]+$ ]] && [[ "$mem_free" =~ ^[0-9]+$ ]]; then
    ...
    # ==================== 4. FreeBSD系统处理 ====================
    if [[ "$OS" == "FreeBSD" ]] && command_exists sysctl; then
        log "FreeBSD系统，使用sysctl获取内存信息"
    ...
    if [[ $total -gt 0 ]]; then
        ...
        if [[ ${diff#-} -gt $tolerance ]]; then
            log "内存数据不一致，进行修正 (总:${total}KB, 已用:${used}KB, 可用:${free}KB, 差异:${diff}KB)"
            ...
            log "修正后: 总=${total}KB, 已用=${used}KB, 可用=${free}KB"
        fi
    ...
    if [[ "$is_container" == "true" ]]; then
        source_type="container_fallback"
    else
        source_type="host_system"
    fi

    echo "{\"total\":$total,\"used\":$used,\"free\":$free,\"usage_percent\":$usage_percent,\"source\":\"$source_type\",\"is_container\":$is_container}"
}
