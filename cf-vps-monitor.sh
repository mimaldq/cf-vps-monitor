#!/bin/bash

# cf-vps-monitor - Cloudflare Worker VPS监控脚本
# 版本: 2.0.0 (FreeBSD优化版)
# 完全兼容FreeBSD系统

set -euo pipefail

# ==================== 全局配置 ====================
readonly SCRIPT_VERSION="2.0.0"
readonly SCRIPT_NAME="cf-vps-monitor"

# 系统检测
OS=$(uname -s)
readonly OS

# 颜色定义
readonly RED='\033[0;31m'
readonly GREEN='\033[0;32m'
readonly YELLOW='\033[1;33m'
readonly BLUE='\033[0;34m'
readonly CYAN='\033[0;36m'
readonly NC='\033[0m'

# 文件路径
readonly SCRIPT_DIR="$HOME/.cf-vps-monitor"
readonly CONFIG_FILE="$SCRIPT_DIR/config/config"
readonly LOG_FILE="$SCRIPT_DIR/logs/monitor.log"
readonly PID_FILE="$SCRIPT_DIR/run/monitor.pid"
readonly SERVICE_FILE="$SCRIPT_DIR/bin/vps-monitor-service.sh"

# ==================== 工具函数 ====================

print_msg() {
    local color="$1"
    local msg="$2"
    printf "%b%s%b\n" "${color}" "${msg}" "${NC}"
}

log() {
    local msg="$1"
    local timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    echo "[$timestamp] $msg" >> "$LOG_FILE"
    [[ "${SERVICE_MODE:-false}" != "true" ]] && echo "[$timestamp] $msg"
}

error_exit() {
    local msg="$1"
    print_msg "$RED" "ERROR: $msg" >&2
    log "ERROR: $msg"
    exit 1
}

command_exists() {
    command -v "$1" >/dev/null 2>&1
}

# FreeBSD兼容的sed函数
safe_sed() {
    local pattern="$1"
    local file="$2"
    
    if [[ "$OS" == "FreeBSD" ]] || [[ "$OS" == "Darwin" ]]; then
        sed -i '' "$pattern" "$file" 2>/dev/null || return 1
    else
        sed -i "$pattern" "$file" 2>/dev/null || return 1
    fi
}

# ==================== 一键安装函数 ====================

# 解析命令行参数
parse_args() {
    local install_mode=false
    local server_id=""
    local api_key=""
    local worker_url=""
    local command=""
    
    while [[ $# -gt 0 ]]; do
        case "$1" in
            -i|--install)
                install_mode=true
                shift
                ;;
            -s|--server-id)
                server_id="$2"
                shift 2
                ;;
            -k|--api-key)
                api_key="$2"
                shift 2
                ;;
            -u|--worker-url)
                worker_url="$2"
                shift 2
                ;;
            -h|--help)
                show_help
                exit 0
                ;;
            *)
                command="$1"
                shift
                ;;
        esac
    done
    
    if [[ "$install_mode" == "true" ]]; then
        if [[ -z "$server_id" || -z "$api_key" || -z "$worker_url" ]]; then
            error_exit "One-click install requires all parameters: -s SERVER_ID -k API_KEY -u WORKER_URL"
        fi
        one_click_install "$server_id" "$api_key" "$worker_url"
        exit $?
    fi
    
    echo "$command"
}

# 一键安装主函数
one_click_install() {
    local server_id="$1"
    local api_key="$2"
    local worker_url="$3"
    
    print_msg "$CYAN" "========================================"
    print_msg "$CYAN" "      VPS Monitor One-Click Install"
    print_msg "$CYAN" "========================================"
    echo
    
    # 显示安装参数
    local masked_api_key="${api_key:0:8}****************${api_key: -8}"
    print_msg "$BLUE" "Install Config:"
    echo "  Server ID: $server_id"
    echo "  API Key: $masked_api_key"
    echo "  Worker URL: $worker_url"
    echo
    
    # 检测系统
    detect_system
    
    # 创建目录
    create_dirs
    
    # 保存配置
    save_config_direct "$server_id" "$api_key" "$worker_url"
    
    # 安装依赖
    install_deps
    
    # 创建服务脚本
    create_service_script
    
    # 启动服务
    if start_service; then
        print_msg "$GREEN" "✓ VPS Monitor installed successfully!"
        echo
        print_msg "$CYAN" "Service Info:"
        echo "  Install Dir: $SCRIPT_DIR"
        echo "  Config File: $CONFIG_FILE"
        echo "  Log File: $LOG_FILE"
        echo "  Service Status: Running"
        echo
        print_msg "$YELLOW" "Management Commands:"
        echo "  Check status: $0 status"
        echo "  View logs: $0 logs"
        echo "  Stop service: $0 stop"
        echo "  Restart service: $0 restart"
        echo
        print_msg "$GREEN" "✓ Monitor service started and reporting data"
    else
        error_exit "Service startup failed"
    fi
}

# ==================== 系统检测函数 ====================

detect_system() {
    print_msg "$BLUE" "Detecting system environment..."
    
    local os_name=$OS
    local os_version=""
    local arch=$(uname -m)
    
    case "$os_name" in
        Linux)
            if [[ -f /etc/os-release ]]; then
                . /etc/os-release
                os_version="$NAME $VERSION"
            else
                os_version="Linux (unknown)"
            fi
            ;;
        Darwin)
            os_version="macOS $(sw_vers -productVersion)"
            ;;
        FreeBSD)
            os_version="FreeBSD $(uname -r)"
            ;;
        *)
            os_version="$os_name"
            ;;
    esac
    
    print_msg "$GREEN" "✓ System: $os_version"
    print_msg "$GREEN" "✓ Architecture: $arch"
}

# ==================== 目录创建函数 ====================

create_dirs() {
    print_msg "$BLUE" "Creating directory structure..."
    
    local dirs=(
        "$SCRIPT_DIR/bin"
        "$SCRIPT_DIR/config"
        "$SCRIPT_DIR/logs"
        "$SCRIPT_DIR/run"
        "$SCRIPT_DIR/tmp"
    )
    
    for dir in "${dirs[@]}"; do
        if mkdir -p "$dir" 2>/dev/null; then
            print_msg "$GREEN" "  ✓ $dir"
        else
            error_exit "Failed to create directory: $dir"
        fi
    done
    
    export TMPDIR="$SCRIPT_DIR/tmp"
    print_msg "$GREEN" "✓ Directory structure created"
}

# ==================== 配置管理函数 ====================

save_config_direct() {
    local server_id="$1"
    local api_key="$2"
    local worker_url="$3"
    
    print_msg "$BLUE" "Saving configuration..."
    
    # 验证输入
    [[ -z "$server_id" ]] && error_exit "Server ID cannot be empty"
    [[ -z "$api_key" ]] && error_exit "API Key cannot be empty"
    [[ -z "$worker_url" ]] && error_exit "Worker URL cannot be empty"
    
    # 清理输入
    server_id=$(echo "$server_id" | tr -d '\r\n\t' | sed 's/[^[:alnum:]_-]//g')
    api_key=$(echo "$api_key" | tr -d '\r\n\t' | sed 's/[^[:alnum:]]//g')
    worker_url=$(echo "$worker_url" | tr -d '\r\n\t')
    
    # 创建配置文件
    mkdir -p "$(dirname "$CONFIG_FILE")"
    cat > "$CONFIG_FILE" << EOF
# VPS Monitor Configuration
WORKER_URL="$worker_url"
SERVER_ID="$server_id"
API_KEY="$api_key"
INTERVAL="10"
EOF
    
    chmod 600 "$CONFIG_FILE"
    print_msg "$GREEN" "✓ Configuration saved"
}

# ==================== 依赖安装函数 ====================

install_deps() {
    print_msg "$BLUE" "Checking system dependencies..."
    
    local missing_deps=()
    
    # 检查curl
    if ! command_exists curl; then
        missing_deps+=("curl")
    fi
    
    # 如果没有缺失依赖
    if [[ ${#missing_deps[@]} -eq 0 ]]; then
        print_msg "$GREEN" "✓ All required dependencies are installed"
        return 0
    fi
    
    print_msg "$YELLOW" "Missing dependencies: ${missing_deps[*]}"
    
    # FreeBSD特殊处理
    if [[ "$OS" == "FreeBSD" ]]; then
        install_deps_freebsd "${missing_deps[@]}"
    elif [[ "$OS" == "Linux" ]]; then
        install_deps_linux "${missing_deps[@]}"
    elif [[ "$OS" == "Darwin" ]]; then
        install_deps_macos "${missing_deps[@]}"
    else
        print_msg "$YELLOW" "Please install manually: ${missing_deps[*]}"
        return 1
    fi
    
    # 验证安装结果
    for dep in "${missing_deps[@]}"; do
        if ! command_exists "$dep"; then
            print_msg "$RED" "✗ $dep installation failed, please install manually"
            return 1
        fi
    done
    
    print_msg "$GREEN" "✓ Dependencies installed"
    return 0
}

install_deps_freebsd() {
    local missing_deps=("$@")
    
    print_msg "$BLUE" "Installing dependencies on FreeBSD..."
    
    if command_exists pkg; then
        print_msg "$CYAN" "Using pkg package manager..."
        for dep in "${missing_deps[@]}"; do
            if ! sudo pkg install -y "$dep" 2>/dev/null; then
                print_msg "$YELLOW" "Failed to install $dep with pkg, trying alternative..."
                case "$dep" in
                    curl)
                        # 尝试从ports安装
                        if command_exists portsnap; then
                            print_msg "$CYAN" "Installing curl from ports..."
                            sudo portsnap fetch extract 2>/dev/null || true
                            cd /usr/ports/ftp/curl && sudo make install clean 2>/dev/null || true
                        fi
                        ;;
                esac
            fi
        done
    else
        print_msg "$RED" "pkg command not found. Please install curl manually:"
        print_msg "$CYAN" "  sudo pkg install curl"
        return 1
    fi
}

install_deps_linux() {
    local missing_deps=("$@")
    
    print_msg "$BLUE" "Installing dependencies on Linux..."
    
    if command_exists apt-get; then
        sudo apt-get update && sudo apt-get install -y "${missing_deps[@]}" || return 1
    elif command_exists yum; then
        sudo yum install -y "${missing_deps[@]}" || return 1
    elif command_exists dnf; then
        sudo dnf install -y "${missing_deps[@]}" || return 1
    elif command_exists pacman; then
        sudo pacman -Sy --noconfirm "${missing_deps[@]}" || return 1
    else
        print_msg "$RED" "No supported package manager found"
        return 1
    fi
}

install_deps_macos() {
    local missing_deps=("$@")
    
    print_msg "$BLUE" "Installing dependencies on macOS..."
    
    if command_exists brew; then
        brew install "${missing_deps[@]}" || return 1
    else
        print_msg "$RED" "Please install Homebrew first: https://brew.sh"
        return 1
    fi
}

# ==================== 服务脚本创建函数 ====================

create_service_script() {
    print_msg "$BLUE" "Creating service script..."
    
    local main_script_path=$(realpath "$0" 2>/dev/null || echo "$0")
    
    cat > "$SERVICE_FILE" << 'EOF'
#!/bin/sh

# VPS Monitor Service Script
# Compatible with FreeBSD

set -e

# Configuration paths
SCRIPT_DIR="__SCRIPT_DIR__"
CONFIG_FILE="$SCRIPT_DIR/config/config"
LOG_FILE="$SCRIPT_DIR/logs/monitor.log"
PID_FILE="$SCRIPT_DIR/run/monitor.pid"

# Service mode flag
export SERVICE_MODE=true

# Ensure directories exist
mkdir -p "$(dirname "$LOG_FILE")" "$(dirname "$PID_FILE")" 2>/dev/null

# Log function
log() {
    local message="$1"
    local timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    echo "[$timestamp] $message" >> "$LOG_FILE"
}

# Load configuration
load_config() {
    if [ -f "$CONFIG_FILE" ]; then
        . "$CONFIG_FILE"
    else
        log "ERROR: Config file not found"
        exit 1
    fi
}

# Get CPU usage (FreeBSD compatible)
get_cpu_usage() {
    local usage=0
    if [ -f /proc/stat ]; then
        # Linux
        local cpu_line=$(head -n1 /proc/stat)
        local idle=$(echo "$cpu_line" | awk '{print $5}')
        local total=0
        for i in $(seq 2 9); do
            local val=$(echo "$cpu_line" | awk -v i="$i" '{print $i}')
            total=$((total + val))
        done
        if [ $total -gt 0 ]; then
            usage=$((100 - (idle * 100 / total)))
        fi
    elif command_exists sysctl; then
        # FreeBSD
        local cpu_times=$(sysctl -n kern.cp_time 2>/dev/null || echo "0 0 0 0 0")
        local idle=$(echo "$cpu_times" | awk '{print $5}')
        local total=0
        for i in 1 2 3 4 5; do
            local val=$(echo "$cpu_times" | awk -v i="$i" '{print $i}')
            total=$((total + val))
        done
        if [ $total -gt 0 ]; then
            usage=$((100 - (idle * 100 / total)))
        fi
    fi
    echo "$usage"
}

# Get memory usage (FreeBSD compatible)
get_memory_usage() {
    local usage=0
    if [ -f /proc/meminfo ]; then
        # Linux
        local total=$(grep "^MemTotal:" /proc/meminfo | awk '{print $2}')
        local free=$(grep "^MemFree:" /proc/meminfo | awk '{print $2}')
        if [ -n "$total" ] && [ "$total" -gt 0 ]; then
            usage=$((100 - (free * 100 / total)))
        fi
    elif command_exists sysctl; then
        # FreeBSD
        local page_size=$(sysctl -n hw.pagesize 2>/dev/null || echo 4096)
        local total_pages=$(sysctl -n vm.stats.vm.v_page_count 2>/dev/null || echo 0)
        local free_pages=$(sysctl -n vm.stats.vm.v_free_count 2>/dev/null || echo 0)
        local total=$((total_pages * page_size / 1024))
        local free=$((free_pages * page_size / 1024))
        if [ $total -gt 0 ]; then
            usage=$((100 - (free * 100 / total)))
        fi
    fi
    echo "$usage"
}

# Get disk usage
get_disk_usage() {
    local usage=0
    if command_exists df; then
        usage=$(df -h / | tail -1 | awk '{print $5}' | sed 's/%//' 2>/dev/null || echo 0)
    fi
    echo "$usage"
}

# Get uptime
get_uptime() {
    local uptime=0
    if [ -f /proc/uptime ]; then
        uptime=$(awk -F. '{print $1}' /proc/uptime 2>/dev/null || echo 0)
    elif command_exists sysctl; then
        local boot_time=$(sysctl -n kern.boottime 2>/dev/null | awk '{print $4}' | tr -d ',')
        local current_time=$(date +%s)
        if [ -n "$boot_time" ]; then
            uptime=$((current_time - boot_time))
        fi
    fi
    echo "$uptime"
}

# Build monitor data
build_monitor_data() {
    local timestamp=$(date +%s)
    local cpu_usage=$(get_cpu_usage)
    local memory_usage=$(get_memory_usage)
    local disk_usage=$(get_disk_usage)
    local uptime=$(get_uptime)
    
    cat << JSON
{
    "timestamp": $timestamp,
    "cpu_usage": $cpu_usage,
    "memory_usage": $memory_usage,
    "disk_usage": $disk_usage,
    "uptime": $uptime,
    "server_id": "$SERVER_ID"
}
JSON
}

# Report data
report_data() {
    local data=$(build_monitor_data)
    
    local response=$(curl -s -w "%{http_code}" -X POST "$WORKER_URL/api/report/$SERVER_ID" \
        -H "Content-Type: application/json" \
        -H "X-API-Key: $API_KEY" \
        -d "$data" 2>/dev/null || echo "000")
    
    local http_code=$(echo "$response" | tail -c 4)
    
    if [ "$http_code" = "200" ]; then
        log "Data reported successfully"
        return 0
    else
        log "Data report failed (HTTP $http_code)"
        return 1
    fi
}

# Main loop
main() {
    log "Monitor service started (PID: $$)"
    echo $$ > "$PID_FILE"
    
    # Signal handling
    trap 'log "Received termination signal, stopping..."; rm -f "$PID_FILE"; exit 0' TERM INT
    
    # Load configuration
    load_config
    
    while true; do
        if report_data; then
            log "Report successful, waiting ${INTERVAL} seconds..."
        else
            log "Report failed, retrying in ${INTERVAL} seconds..."
        fi
        
        sleep "$INTERVAL"
    done
}

# Start main function
main
EOF
    
    # 替换脚本目录路径 - 使用FreeBSD兼容的方法
    if [[ "$OS" == "FreeBSD" ]] || [[ "$OS" == "Darwin" ]]; then
        # FreeBSD/MacOS使用不同的sed语法
        sed -i '' "s|__SCRIPT_DIR__|$SCRIPT_DIR|g" "$SERVICE_FILE"
    else
        sed -i "s|__SCRIPT_DIR__|$SCRIPT_DIR|g" "$SERVICE_FILE"
    fi
    
    chmod +x "$SERVICE_FILE"
    print_msg "$GREEN" "✓ Service script created"
}

# ==================== 服务管理函数 ====================

# 检查服务是否运行
is_service_running() {
    if [ -f "$PID_FILE" ]; then
        local pid=$(cat "$PID_FILE" 2>/dev/null)
        if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
            return 0
        fi
    fi
    
    # 检查进程
    if command_exists pgrep; then
        if pgrep -f "$(basename "$SERVICE_FILE")" >/dev/null 2>&1; then
            return 0
        fi
    else
        # FreeBSD可能没有pgrep，使用ps替代
        if ps aux | grep -v grep | grep "$(basename "$SERVICE_FILE")" >/dev/null 2>&1; then
            return 0
        fi
    fi
    
    return 1
}

# 启动服务
start_service() {
    print_msg "$BLUE" "Starting monitor service..."
    
    # 检查是否已在运行
    if is_service_running; then
        print_msg "$YELLOW" "Monitor service is already running"
        return 0
    fi
    
    # 确保服务脚本存在
    if [ ! -f "$SERVICE_FILE" ]; then
        error_exit "Service script not found: $SERVICE_FILE"
    fi
    
    # 清理旧的PID文件
    rm -f "$PID_FILE" 2>/dev/null || true
    
    # 启动服务
    if command_exists nohup; then
        nohup "$SERVICE_FILE" >> "$LOG_FILE" 2>&1 &
    else
        "$SERVICE_FILE" >> "$LOG_FILE" 2>&1 &
    fi
    
    local pid=$!
    echo "$pid" > "$PID_FILE"
    
    # 验证启动
    sleep 2
    if kill -0 "$pid" 2>/dev/null; then
        print_msg "$GREEN" "✓ Monitor service started (PID: $pid)"
        
        # 配置自启动
        setup_autostart
        
        return 0
    else
        print_msg "$RED" "✗ Monitor service failed to start"
        if [ -f "$LOG_FILE" ]; then
            print_msg "$YELLOW" "Check logs: tail -f $LOG_FILE"
        fi
        rm -f "$PID_FILE" 2>/dev/null
        return 1
    fi
}

# 配置自启动
setup_autostart() {
    print_msg "$BLUE" "Configuring auto-start..."
    
    # FreeBSD使用rc.d
    if [[ "$OS" == "FreeBSD" ]]; then
        setup_rc_daemon
    elif command_exists systemctl; then
        setup_systemd_service
    elif command_exists crontab; then
        setup_crontab_autostart
    else
        print_msg "$YELLOW" "⚠ Auto-start configuration failed, service needs manual start"
    fi
}

# FreeBSD rc.d配置
setup_rc_daemon() {
    print_msg "$CYAN" "Setting up FreeBSD rc.d service..."
    
    local rc_file="/usr/local/etc/rc.d/cf-vps-monitor"
    
    if [ $EUID -eq 0 ]; then
        cat > "$rc_file" << 'RCFILE'
#!/bin/sh

# PROVIDE: cf_vps_monitor
# REQUIRE: NETWORKING
# KEYWORD: shutdown

. /etc/rc.subr

name="cf_vps_monitor"
rcvar="${name}_enable"
pidfile="/var/run/cf-vps-monitor.pid"
command="/usr/sbin/daemon"
command_args="-P ${pidfile} -r -f __SERVICE_SCRIPT__"

load_rc_config $name
run_rc_command "$1"
RCFILE
        
        # 替换服务脚本路径
        sed -i '' "s|__SERVICE_SCRIPT__|$SERVICE_FILE|g" "$rc_file"
        chmod +x "$rc_file"
        
        # 启用服务
        sysrc cf_vps_monitor_enable="YES"
        print_msg "$GREEN" "✓ FreeBSD rc.d service configured"
    else
        print_msg "$YELLOW" "Need root privileges to setup rc.d service"
        setup_crontab_autostart
    fi
}

# 配置systemd服务
setup_systemd_service() {
    local service_file=""
    
    if [ $EUID -eq 0 ]; then
        service_file="/etc/systemd/system/cf-vps-monitor.service"
    else
        service_file="$HOME/.config/systemd/user/cf-vps-monitor.service"
        mkdir -p "$(dirname "$service_file")"
    fi
    
    cat > "$service_file" << EOF
[Unit]
Description=CF VPS Monitor Service
After=network.target

[Service]
Type=simple
ExecStart=$SERVICE_FILE
Restart=always
RestartSec=10
WorkingDirectory=$SCRIPT_DIR

[Install]
WantedBy=default.target
EOF
    
    if [ $EUID -eq 0 ]; then
        systemctl daemon-reload
        systemctl enable cf-vps-monitor.service
    else
        systemctl --user daemon-reload
        systemctl --user enable cf-vps-monitor.service
    fi
    
    print_msg "$GREEN" "✓ systemd service configured"
}

# 配置crontab自启动
setup_crontab_autostart() {
    if ! command_exists crontab; then
        return 1
    fi
    
    local crontab_entry="@reboot sleep 30 && $SERVICE_FILE"
    (crontab -l 2>/dev/null | grep -v "$SERVICE_FILE"; echo "$crontab_entry") | crontab -
    
    print_msg "$GREEN" "✓ crontab auto-start configured"
}

# 停止服务
stop_service() {
    print_msg "$BLUE" "Stopping monitor service..."
    
    # 停止FreeBSD rc.d服务
    if [[ "$OS" == "FreeBSD" ]] && [ $EUID -eq 0 ]; then
        service cf_vps_monitor stop 2>/dev/null || true
        sysrc -x cf_vps_monitor_enable 2>/dev/null || true
        rm -f /usr/local/etc/rc.d/cf-vps-monitor 2>/dev/null || true
    fi
    
    # 停止systemd服务
    if command_exists systemctl; then
        if [ $EUID -eq 0 ]; then
            systemctl stop cf-vps-monitor.service 2>/dev/null || true
            systemctl disable cf-vps-monitor.service 2>/dev/null || true
        else
            systemctl --user stop cf-vps-monitor.service 2>/dev/null || true
            systemctl --user disable cf-vps-monitor.service 2>/dev/null || true
        fi
    fi
    
    # 停止进程
    if [ -f "$PID_FILE" ]; then
        local pid=$(cat "$PID_FILE" 2>/dev/null)
        if [ -n "$pid" ]; then
            kill "$pid" 2>/dev/null || true
            sleep 1
            kill -9 "$pid" 2>/dev/null || true
        fi
    fi
    
    # 清理crontab
    if command_exists crontab; then
        (crontab -l 2>/dev/null | grep -v "$SERVICE_FILE") | crontab - 2>/dev/null || true
    fi
    
    # 清理文件
    rm -f "$PID_FILE" 2>/dev/null || true
    
    print_msg "$GREEN" "✓ Monitor service stopped"
}

# 检查服务状态
check_service_status() {
    print_msg "$BLUE" "Monitor Service Status"
    echo
    
    if is_service_running; then
        local pid=""
        if [ -f "$PID_FILE" ]; then
            pid=$(cat "$PID_FILE" 2>/dev/null)
        elif command_exists pgrep; then
            pid=$(pgrep -f "$(basename "$SERVICE_FILE")" | head -1)
        fi
        
        print_msg "$GREEN" "✓ Monitor service is running"
        echo "  Process PID: $pid"
        
        # 显示运行时间
        if [ -n "$pid" ]; then
            if command_exists ps; then
                local etime=$(ps -p "$pid" -o etime= 2>/dev/null || echo "unknown")
                echo "  Uptime: $etime"
            fi
        fi
    else
        print_msg "$RED" "✗ Monitor service is not running"
    fi
    
    # 配置文件状态
    echo
    print_msg "$BLUE" "Configuration:"
    if [ -f "$CONFIG_FILE" ]; then
        local worker_url=$(grep '^WORKER_URL=' "$CONFIG_FILE" | cut -d= -f2 | tr -d '"')
        local server_id=$(grep '^SERVER_ID=' "$CONFIG_FILE" | cut -d= -f2 | tr -d '"')
        local api_key=$(grep '^API_KEY=' "$CONFIG_FILE" | cut -d= -f2 | tr -d '"')
        local interval=$(grep '^INTERVAL=' "$CONFIG_FILE" | cut -d= -f2 | tr -d '"')
        
        echo "  Worker URL: ${worker_url:0:50}..."
        echo "  Server ID: $server_id"
        echo "  API Key: ${api_key:0:8}..."
        echo "  Report Interval: ${interval} seconds"
    else
        print_msg "$YELLOW" "✗ Config file not found"
    fi
    
    # 日志状态
    echo
    print_msg "$BLUE" "Log File:"
    if [ -f "$LOG_FILE" ]; then
        local log_size=""
        if command_exists du; then
            log_size=$(du -h "$LOG_FILE" 2>/dev/null | cut -f1)
        fi
        local log_lines=0
        if command_exists wc; then
            log_lines=$(wc -l < "$LOG_FILE" 2>/dev/null || echo "0")
        fi
        echo "  Location: $LOG_FILE"
        [ -n "$log_size" ] && echo "  Size: $log_size"
        echo "  Lines: $log_lines"
        
        # 显示最后3行日志
        echo "  Last log entries:"
        tail -n 3 "$LOG_FILE" 2>/dev/null | sed 's/^/    /' || true
    else
        print_msg "$YELLOW" "✗ Log file not found"
    fi
}

# 查看日志
view_logs() {
    if [ ! -f "$LOG_FILE" ]; then
        print_msg "$YELLOW" "Log file not found"
        return 1
    fi
    
    print_msg "$BLUE" "Monitor Service Logs (Ctrl+C to exit)"
    echo "========================================"
    tail -f "$LOG_FILE"
}

# 重启服务
restart_service() {
    stop_service
    sleep 2
    start_service
}

# 卸载服务
uninstall_service() {
    print_msg "$YELLOW" "WARNING: This will completely uninstall VPS Monitor"
    echo -n "Confirm uninstall? (y/N): "
    read -r confirm
    
    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
        print_msg "$BLUE" "Uninstall cancelled"
        return 0
    fi
    
    print_msg "$BLUE" "Starting uninstall..."
    
    # 停止服务
    stop_service
    
    # 删除安装目录
    if [ -d "$SCRIPT_DIR" ]; then
        rm -rf "$SCRIPT_DIR"
        print_msg "$GREEN" "✓ VPS Monitor completely uninstalled"
    else
        print_msg "$YELLOW" "Install directory not found"
    fi
}

# ==================== 帮助信息 ====================

show_help() {
    cat << EOF
VPS Monitor Script v$SCRIPT_VERSION

One-Click Install:
  wget https://raw.githubusercontent.com/mimaldq/cf-vps-monitor/main/cf-vps-monitor.sh -O cf-vps-monitor.sh && chmod +x cf-vps-monitor.sh && ./cf-vps-monitor.sh -i -k API_KEY -s SERVER_ID -u WORKER_URL

Commands:
  start           Start monitor service
  stop            Stop monitor service
  restart         Restart monitor service
  status          Check service status
  logs            View service logs
  uninstall       Uninstall monitor service
  help            Show this help

Options:
  -i, --install      One-click install mode
  -s, --server-id    Server ID
  -k, --api-key      API Key
  -u, --worker-url   Worker URL
  -h, --help         Show help

Examples:
  # One-click install
  ./cf-vps-monitor.sh -i -k 9947e755... -s hr20js -u https://mycf-vps.workers.dev
  
  # Check status
  ./cf-vps-monitor.sh status
  
  # View logs
  ./cf-vps-monitor.sh logs

Notes:
  - Script auto-detects system and installs dependencies
  - Service auto-starts on system reboot
  - Default report interval is 10 seconds
EOF
}

# ==================== 主函数 ====================

main() {
    # 解析参数
    local command
    command=$(parse_args "$@")
    
    # 如果没有命令，显示菜单
    if [ -z "$command" ]; then
        show_interactive_menu
        return
    fi
    
    # 处理命令
    case "$command" in
        start)
            start_service
            ;;
        stop)
            stop_service
            ;;
        restart)
            restart_service
            ;;
        status)
            check_service_status
            ;;
        logs)
            view_logs
            ;;
        uninstall)
            uninstall_service
            ;;
        help)
            show_help
            ;;
        *)
            print_msg "$RED" "Unknown command: $command"
            show_help
            exit 1
            ;;
    esac
}

# 交互式菜单
show_interactive_menu() {
    while true; do
        echo
        print_msg "$CYAN" "VPS Monitor Management"
        echo
        echo "1) Start monitor service"
        echo "2) Stop monitor service"
        echo "3) Restart monitor service"
        echo "4) Check service status"
        echo "5) View service logs"
        echo "6) Uninstall monitor service"
        echo "7) Exit"
        echo
        echo -n "Select (1-7): "
        read -r choice
        
        case "$choice" in
            1) start_service ;;
            2) stop_service ;;
            3) restart_service ;;
            4) check_service_status ;;
            5) view_logs ;;
            6) uninstall_service ;;
            7) exit 0 ;;
            *) print_msg "$RED" "Invalid selection, please try again" ;;
        esac
        
        echo
        echo -n "Press Enter to continue..."
        read -r
    done
}

# 脚本入口
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    main "$@"
fi
